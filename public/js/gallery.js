...
function initUploader() {
  galleryForm.onchange = function (event) {
    ...
    var galleryFiles = document.getElementById('file-upload');
    var data = new FormData(galleryForm);
    Array.from(galleryFiles.files).forEach(function (file) {
...
var uploadFiles = function uploadFiles(data) {
  var config = {
    onUploadProgress: function onUploadProgress(progressEvent) {
      ...
      $('.progress .progress-bar').css('width', percentCompleted + '%');
    }
  };
  fileProgress.show();
  axios__WEBPACK_IMPORTED_MODULE_0___default().post(galleryForm.action, data, config).then(function (res) {
