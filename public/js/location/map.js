...
function initLocationMap() {
  ...
  var imgWidth = mapImage.width(); // If the image hasn't finished loading, let's try again in half a second
  ...
  initMapModeSwitch();
  initMovePoints();
  disableMovePoints(); //initAddPoints();
...
function mapZoom(change) {
  ...
  if (change > 0) {
    ...
    mapDraggable.css('left', pos.left - mapMouseX / mapElement.width() * 100).css('top', pos.top - mapMouseY / mapElement.height() * 100) //.css('top', pos.top - (mapMouseY * 0.1))
    ;
  } else if (change < 0) {
    mapDraggable.css('left', pos.left + mapMouseX / mapElement.width() * 100).css('top', pos.top + mapMouseY / mapElement.height() * 100) //.css('top', pos.top + (mapMouseY * 0.1))
    ;
...
function loadMapPoint(element) {
  ...
  mapPanelLoader.show();
  mapPanelTarget.html(''); // Show the point as being focused, removing any previously focused point
...
function addPointMove(point) {
  point.draggable({
    ...
    stop: function stop(event, ui) {
      ...
      $.post(location.data('url-move'), data).done(function (result, textStatus, xhr) {//event.preventDefault();
      ...
      setTimeout(function () {
        mapPointIsMoving = false;
      }, 50); //$(this).removeClass('ui-draggable-dragging');
...
function initMapScroll() {
  $(mapImage).bind('wheel', function (event) {
    if (event.ctrlKey === true) {
      event.preventDefault(); // console.log('wheel', event.originalEvent.deltaY);
...
function resizeMapToPage() {
  ...
  $('.loading-map').hide();
  var imgHeight = mapImage.height(); //console.log('img width, height', imgWidth, imgHeight);
  // Get the view box width and height

  var mapWidth = mapElement.width();
  var mapHeight = mapElement.height(); //console.log('div width, height', mapWidth, mapHeight);
...
function formatState(state) {
  ...
  var icon = element.data('icon'); // If there is no icon, use the id
...
function initMapLegend() {
  $(".map-point-legend").click(function () {
    ...
    var target = $($(this).attr('href')); // let targetPosition = target.position();
