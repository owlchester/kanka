...
it('returns the full explore payload for a simple map', function () {
    ...
    $response = $this->get(route('entities.map-api', [1, $map->entity]))
        ...
        ->assertJsonStructure([
            ...
            'i18n' => ['legend_title', 'legend_search', 'ungrouped', 'loading', 'error_load', 'error_delete', 'error_save', 'error_name_required', 'from_entry', 'linked_entry', 'description', 'add_description', 'edit_description', 'description_expand', 'cancel', 'edit_marker', 'center', 'duplicate', 'delete_marker', 'delete_confirm', 'new_pin', 'name_placeholder', 'save', 'save_changes', 'details', 'less', 'premium_custom_icon', 'custom_icon_or_svg', 'custom_icon_helper', 'custom_icon_placeholder', 'markers_count_one', 'markers_count_other', 'peek_map', 'peek_panel', 'toolbar' => ['rapid', 'pin', 'text', 'area', 'circle', 'path', 'helper' => ['pin', 'text', 'area', 'circle', 'path']], 'header' => ['overview', 'settings', 'edit'], 'settings' => ['title', 'grid', 'zoom_min', 'zoom_max', 'zoom_initial', 'distance_name', 'distance_measure', 'center', 'center_coordinates', 'center_marker', 'pick_on_map', 'picking', 'no_marker', 'save', 'error_save'], 'presence' => ['role_edit', 'role_view', 'error_unavailable', 'error_connecting', 'error_disconnected']],
        ]);
...
it('exposes real map zoom limits in the explore payload', function () {
    $this->asUser()->withCampaign();
    ...
    expect($response->json('map.min_zoom'))->toBe(0)
        ->and($response->json('map.max_zoom'))->toBe(15)
        ->and($response->json('map.initial_zoom'))->toBe(15)
        ->and($response->json('i18n.settings.zoom_max_value'))->toBe(15)
        ->and($response->json('i18n.settings.zoom_initial_help'))
        ->toBe(__('maps.helpers.initial_zoom', ['min' => 0, 'max' => 15, 'default' => 12]));
});
