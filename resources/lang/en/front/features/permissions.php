...
return [
    ...
    'first' => 'Kanka\'s permissions is split off into several concepts: role permissions and entity permissions. By default, each campaign comes with an Admin, a Public and a Player role. Members of the admin role can see and do everything in a campaign. The public role is used if the campaign is public and the user isn\'t part of it, and by default the role has no permissions. Lastly, the player role also comes by default with no permissions, and is used for anyone in it. When you invite a new member to your campaign, you define what role they join as. You can create more roles, and move people around, as well as have members in several roles.',
    'second' => 'A role can be set up in different ways. You can for example allow members of a role to view and create characters, but not edit or delete them. If a user can create an entity but can\'t modify it, they will automatically get the update permissions.',
