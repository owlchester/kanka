...
<div class="entities-grid flex flex-wrap gap-3 lg:gap-5">
    @if (!empty($parent))
        <a href="{{ route($route, array_merge($parent->parent ? [$campaign, 'parent_id' => $parent->parent->id] : [$campaign], isset($entityType) && $entityType->isSpecial() ? ['entityType' => $entityType] : [])) }}" class="entity w-[47%] xs:w-[25%] sm:w-48 overflow-hidden rounded flex flex-col shadow-sm hover:shadow-md sm">
        ...
        @includeWhen(!isset($entityType) || $entityType->isSpecial(), 'entities.index._entity', ['model' => $parent, 'isParent' => true])
        @includeWhen(isset($entityType) && !$entityType->isSpecial(), 'cruds.datagrids._grid', ['model' => $parent, 'isParent' => true])
    @endif
    @forelse ($models as $model)
        @includeWhen(!isset($entityType) || $entityType->isSpecial(), 'entities.index._entity')
        @includeWhen(isset($entityType) && !$entityType->isSpecial(), 'cruds.datagrids._grid')
    @empty
        <p class="italic">
