...
@if ($model->image)
    <a href="{{ $model->getOriginalImageUrl() }}" title="{{ $model->name }}" target="_blank">
        <img class="profile-user-img img-responsive img-bordered-sm" src="{{ $model->thumbnail(0) }}" alt="{{ $model->name }} picture">
    </a>
@elseif ($campaignService->campaign()->superboosted() && $model->entity && $model->entity->image)
