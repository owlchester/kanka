...
<table class="table table-hover m-0 mb-2 w-full shadow-xs bg-box rounded " data-render="datagrid2">
    ...
    @forelse ($rows as $row)
        @if ($row instanceof \App\Models\MiscModel && empty($row->entity))
            @continue
        @endif
        <tr class="{{ method_exists($row, 'rowClasses') ? $row->rowClasses() : null }} @if (Datagrid::isHighlighted($row)) warning row-highlighted @endif" @if (method_exists($row, 'rowAttributes')) {!! $row->rowAttributes() !!} @endif>
