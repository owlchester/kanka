...
<a class="entity-image" style="background-image: url('{{ $model->thumbnail() }}');" title="{{ $model->name }}" href="{{ $model->getLink() }}"></a>
@elseif (!empty($with))
    @php $target = \Illuminate\Support\Arr::get($with, 'target', false); @endphp
    <a class="entity-image" style="background-image: url('{{ $model->$target->thumbnail() }}');" title="{{ $model->$target->name }}" href="{{ $model->$target->getLink() }}"></a>
@endif
