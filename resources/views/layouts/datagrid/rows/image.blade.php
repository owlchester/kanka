...
@elseif (!empty($with))
    ...
    <a class="entity-image w-10 h-10 cover-background" style="background-image: url('{{ \App\Facades\Avatar::entity($model->$target->entity)->size(40)->fallback()->thumbnail() }}');" title="{{ $model->$target->name }}" href="{{ $model->$target->getLink($campaign) }}"></a>
@endif
