...
@if (auth()->check() && !auth()->user()->settings()->get('das2'))
<div class="alert alert-warning p-2 flex gap-2 banner-notification " id="banner-notification">
    ...
    @if (auth()->check())
        <button type="button" class="close p-2 banner-notification-dismiss text-2xl" data-dismiss="#banner-notification" aria-hidden="true" data-url="{{ route('settings.banner', ['code' => 'das2']) }}">×</button>
    @endif
</div>
@endif
