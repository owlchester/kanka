    ...
    <div class="header-buttons flex gap-2 items-center justify-end flex-wrap">
        @if ($mode === \App\Enums\Descendants::Direct)
            <x-toggles.filter-button
                route="{{ route('creatures.creatures', [$campaign, $model, 'm' => \App\Enums\Descendants::All]) }}"
                :count="$model->descendants()->has('entity')->count()"
                all
            />
        @else
            <x-toggles.filter-button
                route="{{ route('creatures.creatures', [$campaign, $model, 'm' => \App\Enums\Descendants::Direct]) }}"
