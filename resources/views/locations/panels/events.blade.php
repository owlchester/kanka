<div class="box box-flat">
    <div class="box-body">
        ...
        <?php  $r = $model->events()->acl(auth()->user())->orderBy('name', 'ASC')->with(['location'])->paginate(); ?>
        ...
        <table id="events" class="table table-hover {{ $r->count() === 0 ? 'export-hidden' : '' }}">
            ...
            @foreach ($r as $event)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('events.show', $event->id) }}" data-toggle="tooltip" title="{{ $event->tooltip() }}">{{ $event->name }}</a>
                    </td>
                    <td>{{ $event->type }}</td>
                    <td class="text-right">
