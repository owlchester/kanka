<div class="box box-flat">
    <div class="box-body">
        ...
        <?php  $r = $model->organisations()->acl(auth()->user())->orderBy('name', 'ASC')->with(['members'])->paginate(); ?>
        ...
        <table id="items" class="table table-hover {{ $r->count() === 0 ? 'export-hidden' : '' }}">
            ...
            @foreach ($r as $org)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('organisations.show', $org->id) }}" data-toggle="tooltip" title="{{ $org->tooltip() }}">{{ $org->name }}</a>
                    </td>
                    <td>{{ $org->type }}</td>
