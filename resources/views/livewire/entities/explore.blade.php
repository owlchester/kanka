<div>
    <div class="flex gap-1 items-start">
        <div class="entities-grid flex flex-wrap gap-3 lg:gap-5">
            @if (isset($parent))
                <span role="button" wire:click="toParent()" class="entity w-[47%] xs:w-[25%] sm:w-48 overflow-hidden rounded flex flex-col shadow-sm hover:shadow-md sm">
                ...
                <livewire:entities.entity-card :entity="$parent" :campaign="$campaign" :isParent="true" :key="$parent->id" />
            @endif
            @forelse ($entities as $model)
                <livewire:entities.entity-card :entity="$model" :campaign="$campaign" :key="$model->id" />
            @empty
                <p class="italic">
            ...
            @if (!isset($skipPaginationHelper) && auth()->check() && $entities instanceof \Illuminate\Pagination\LengthAwarePaginator && $entities->hasPages() && !UserCache::dismissedTutorial('pagination'))
                <div class="block border rounded shadow-xs hover:shadow-md w-48 overflow-hidden tutorial pagination-tutorial">
                    <div class="bg-blue-100 h-48 w-48 overflow-hidden p-2 flex flex-col gap-2">
                        <a class="grow" href="{{ route('settings.appearance', ['highlight' => 'pagination', 'from' => base64_encode(route('entities.index', [$campaign, $entityType]))]) }}">
