...
<div class="">
    <div class="entity-asset asset-link flex justify-center items-center overflow-hidden mb-4">
        <a href="{{ route('entities.entity_assets.go', [$campaign, $entity, $asset]) }}" target="_blank" class="w-1/2 h-20 block text-center icon rounded flex items-center align-center justify-center bg-black/10 ">
        ...
        <div class="w-1/2 text truncate p-2">
            ...
            @if(auth()->check() && auth()->user()->can('update', $entity->child))
                <a href="#" data-toggle="ajax-modal" data-target="#entity-modal" data-url="{{ route('entities.entity_assets.edit', [$campaign, $entity, $asset]) }}">
