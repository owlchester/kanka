...
@if (count($models) > 0)
    @foreach ($models as $relation)
        ...
        <li class="list-group-item list-group-item-repeat pinned-relation" data-relation="{{ $relation->target->name }}" data-target="{{ $relation->target_id }}">
        ...
        @else
        <li class="list-group-item pinned-relation" data-target="{{ $relation->target_id }}" data-relation="{{ $relation->target->name }}" >
            <strong>
                {{ $relation->relation }}
            </strong>
            <span class="pull-right">
