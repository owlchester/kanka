...
@foreach ($distinctCalendars as $calendarId => $calendarEvents)
    ...
    @if (!empty($birth) && !empty($death))
        <div class="element profile-life">
            ...
            <a href="{{ $birth->calendar->getLink() }}?year={{ $birth->year }}&month={{ $birth->month }}" data-title="{{ $birth->hasEra() ? $birth->readableRawDate() : $birth->calendar->name }}" data-toggle="tooltip" class="text-link">
                {{ $birth->readableDate() }}
            </a> &#10013; <a href="{{ $death->calendar->getLink() }}?year={{ $death->year }}&month={{ $death->month }}" data-title="{{ $death->hasEra() ? $death->readableRawDate() : $death->calendar->name }}" data-toggle="tooltip" class="text-link">
                {{ $death->readableDate() }}
    ...
    @elseif (!empty($death))
        <div class="element profile-life">
            ...
            <a href="{{ $death->calendar->getLink() }}?year={{ $death->year }}&month={{ $death->month }}" data-title="{{ $death->hasEra() ? $death->readableRawDate() : $death->calendar->name }}" data-toggle="tooltip" class="text-link">
            {{ $death->readableDate() }}
            </a> (&#10013;{{ $death->calcElapsed() }})
        </div>
    @endif
