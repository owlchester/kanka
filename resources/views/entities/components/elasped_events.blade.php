...
@foreach ($distinctCalendars as $calendarId => $calendarEvents)
    ...
    @if (!empty($birth) && !empty($death))
        <li class="flex">
            ...
            <div class="grow text-right">
                <a href="{{ route('entities.show', [$campaign, $birth->calendar->entity]) }}?year={{ $birth->year }}&month={{ $birth->month }}" data-title="{{ $birth->calendar->entity->name }}" data-toggle="tooltip" class="text-link">
                    {{ $birth->readableDate() }}
                </a> &#10013; <a href="{{ route('entities.show', [$campaign, $death->calendar->entity]) }}?year={{ $death->year }}&month={{ $death->month }}" data-title="{{ $death->calendar->entity->name }}" data-toggle="tooltip" class="text-link">
                    {{ $death->readableDate() }}
    ...
    @elseif (!empty($death))
        <li class="flex">
            ...
            <div class="grow text-right">
                <a href="{{ route('entities.show', [$campaign, $death->calendar->entity]) }}?year={{ $death->year }}&month={{ $death->month }}" data-title="{{ $death->calendar->entity->name }}" data-toggle="tooltip" class="text-link">
                {{ $death->readableDate() }}
                </a> ({{ $death->calcElapsed() }})
            </div>
        </li>
