...
@foreach ($entities as $entity)
    <div class="flex">
        <a class="entity-image" style="background-image: url('{{ ($campaign->superboosted() && !empty($entity->image_uuid) && !empty($entity->image) ? Img::crop(40, 40)->url($entity->image->path) : $entity->avatar(true)) }}');"
           title="{{ $entity->name }}"
           href="{{ $entity->url() }}">
...
<div class="text-center">
    <a href="#" class="text-center widget-recent-more"
       data-url="{{ route('dashboard.recent', ['campaign' => $campaign, 'id' => $widget->id, 'page' => $entities->currentPage() + 1]) }}">
