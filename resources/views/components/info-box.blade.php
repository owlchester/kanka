<x-box class="flex items-center gap-5 rounded-xl shadow-xs">
    ...
    @if ($url)
        @if ($ajax)
            <div class="rounded-full border border-base-300  h-12 w-12 flex items-center justify-center cursor-pointer hover:bg-base-200" data-target="{{ $target }}" data-url="{{ $url }}" data-toggle="dialog" @if ($urlTooltip) data-tooltip data-title="{{ $urlTooltip }}" @endif>
