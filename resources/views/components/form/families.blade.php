...
<select multiple="multiple" name="families[]" class="w-full select2 join-item" data-tags="true" style="width: 100%" data-url="{{ route('search-list', [$campaign, config('entities.ids.family')]) }}" data-new-tag="{{ __('crud.actions.new') }}" data-allow-clear="true" data-allow-new="{{ ($dynamicNew || $quickCreator) ? 'true' : 'false' }}" data-placeholder="" id="{{ $fieldUniqIdentifier }}">
