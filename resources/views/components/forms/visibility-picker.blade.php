        ...
        <div class="flex flex-col gap-0.5 p-1 min-w-72">
            @foreach ($options as $value => $name)
                <button
                    ...
                    class="visibility-picker-option flex items-start gap-2.5 p-1.5 px-3 rounded-lg cursor-pointer text-left transition-colors hover:bg-base-200/50 {{ $value === $selected ? 'bg-primary/5 ring-1 ring-primary/30' : '' }}"
                    data-value="{{ $value }}"
