<div class="grid grid-cols-2 gap-4 text-slate-900">
    ...
    <div class="rounded-2xl p-4 bg-blue-100">
        <p class="text-xl font-semibold text-blue-700 tracking-tight">{{ $campaign->members->count() }}</p>
        <p class="text-xs">{{ __('settings/premium.create.recap.instant') }}</p>
    </div>
    <div class="rounded-2xl p-4 bg-red-100">
        <p class="text-xl font-semibold text-red-700 tracking-tight">{{ config('limits.filesize.image.standard') }}MiB → {{  config('limits.filesize.image.owlbear') }}MiB</p>
        <p class="text-xs">{{ __('settings/premium.create.recap.size') }}</p>
    </div>
    @else
    <div class="rounded-2xl p-4 bg-blue-100">
        <p class="text-xl font-semibold text-blue-700 tracking-tight">{{ __('settings/premium.create.recap.control.title') }}</p>
        ...
        @endphp
    <div class="rounded-2xl p-4 bg-red-100">
        <p class="text-xl font-semibold text-red-700 tracking-tight">{{ round(config('limits.gallery.standard')/1024, 2) }}MiB → {{ $base }}GiB</p>
        <p class="text-xs">{{ __('settings/premium.create.recap.gallery') }}</p>
    </div>
    @endif
    <div class="rounded-2xl p-4 bg-amber-100">
        <p class="text-xl font-semibold text-amber-700 tracking-tight">{{ __('settings/premium.create.recap.recovery.title', ['amount' => config('entities.hard_delete')]) }}</p>
        <p class="text-xs">{{ __('settings/premium.create.recap.recovery.description') }}</p>
    </div>
    <div class="rounded-2xl p-4 bg-green-100">
        <p class="text-xl font-semibold text-green-700 tracking-tight">12,000+</p>
