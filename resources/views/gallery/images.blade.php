...
@if(!empty($folder))
    <li tabindex="0" data-folder="{{ empty($folder->folder_id) ? route('campaign.gallery.index', $campaign) : route('campaign.gallery.index', [$campaign, 'folder_id' => $folder->folder_id]) }}" class="block overflow-hidden rounded shadow-sm aspect-square w-[47%] xs:w-[25%] sm:w-48 cursor-pointer flex flex-col bg-box select-none hover:shadow-md focus:shadow-md items-center justify-center">
