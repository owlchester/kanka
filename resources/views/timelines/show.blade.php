@section('entity-header-actions-override')
    <div class="header-buttons">
        ...
        @can('update', $model)
            <a href="{{ route('timelines.reorder', [$campaign, $model->campaign_id, $model]) }}" class="btn btn-default btn-sm ">
                <i class="fa-solid fa-sort"></i> {{ __('timelines.show.tabs.reorder') }}
            </a>
            <a href="{{ route('timelines.edit', [$campaign, $model]) }}" class="btn btn-primary btn-sm ">
                <i class="fa-solid fa-pencil"></i> {{ __('crud.edit') }}
            </a>
        @endcan
        @can('post', [$model, 'add'])
            <a href="{{ route('entities.posts.create', ['campaign' => $model->campaign_id, 'entity' => $model->entity->id]) }}" class="btn btn-warning btn-sm btn-new-entity"
