    ...
    @foreach ($map->legendMarkers(false) as $marker)
        <li>
            @if(isset($marker['markers']))
                <a href="#" class="map-legend-marker map-legend-group" data-toggle="collapse" data-target="#map-legend-group-{{ $marker['id'] }}">{!! stripcslashes($marker['name']) !!}</a>

                <ul class="collapse in" id="map-legend-group-{{ $marker['id'] }}">
                    @foreach ($marker['markers'] as $mk)
                        <li>
                            <a href="#" class="map-legend-marker" data-lng="{{ $mk['longitude'] }}" data-lat="{{ $mk['latitude'] }}" data-id="marker{{ $mk['id'] }}">{!! stripcslashes($mk['name']) !!}</a>
                        </li>
                    @endforeach
                </ul>
            @else
