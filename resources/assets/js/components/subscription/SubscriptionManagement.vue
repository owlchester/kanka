<template>
    ...
    && paymentMethods.length > 0">
        ...
        <div v-show="paymentMethodSelected">
            ...
            <div class="alert alert-danger" v-show="showSaveError">
                <h4>{{ saveError.title }}</h4>
                {{ saveError.text }}
            </div>
                ...
                <div v-for="plan in plans" class="col-xs-6">
                    <div class="box box-widget widget-user widget-patron">
                        <div v-bind:class="planHeaderClass(plan)">
                            ...
                            <h5 class="widget-user-desc">{{ currency }}{{ plan.price}}</h5>
                        </div>
                        ...
                        <div class="box-body">
                            ...
                            <div class="text-center">
                                <button v-show="savingSubscriptionStatus == 0" class="btn btn-primary btn-block mt-3" id="add-card-button" v-on:click="updateSubscription(plan.key)">
                                    {{ $t('settings.subscription.subscription.actions.subscribe' )}}
                                </button>
                                <button v-show="savingSubscriptionStatus != 0" class="btn btn-primary btn-block " disabled="disabled">
            ...
            updateSubscription(planId) {
                ...
                axios.put('/subscription-api/subscription', {
                    ...
                    if (response.data.subscription_updated) {
                        ...
                        this.savingSubscriptionStatus = 0;
                        this.saveError = {title: response.data.title, text: response.data.text};
