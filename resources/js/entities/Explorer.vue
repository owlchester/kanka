    ...
    <div class="flex flex-col gap-4 w-full" v-else>
        <div class="flex gap-2 justify-between items-center flex-wrap ">
            <div class="flex gap-1 items-center flex-wrap ">
                ...
                <div v-else>
                    <i class="fa-regular fa-filter text-lg rounded-lg hover:text-primary-content cursor-pointer px-2 py-1 hover:bg-primary" v-bind:aria-label="i18n.filters" :title="i18n.filters" @click="openFilters">
                ...
                <button
                    ...
                    class="rounded-full border-primary text-primary border px-3 py-1 text-xs uppercase hover:bg-primary hover:text-primary-content"
                    v-html="i18n.select"></button>
                <button
                    v-if="hasPermissions() && selecting"
                    @click="toggleSelecting()"
                    class="rounded-full bg-primary text-primary-content px-3 py-1 text-xs uppercase"
            ...
            <div class="flex gap-2 items-center" v-if="!selecting">
                ...
                <div class="join"  v-if="hasPermissions() && permissions.create">
                    ...
                    <div v-if="permissions.template">
                        <button ref="templateBtn" type="button" class="btn2 btn-primary join-item" aria-expanded="false" aria-label="Create from template" aria-haspopup="menu" aria-controls="templates-submenu">
        ...
        <div ref="hiddenMenus" style="display: none">
            <div ref="orderMenu" class="flex flex-col gap-1">
                <button @click="orderBy('name')" :class="orderByClass('name')" class="px-2 py-2 hover:bg-base-200 rounded-xl flex items-center gap-1.5 text-sm transition-all duration-150  text-base-content ">
                    <i v-if="isOrdering('name')" :class="orderByIcon('name')"></i>
                    <span v-html="i18n.fields['name']"></span>
                </button>
                <button @click="orderBy('type')" :class="orderByClass('type')" class="px-2 py-2 hover:bg-base-200 rounded-xl flex items-center gap-1.5 text-sm transition-all duration-150  text-base-content">
                    <i v-if="isOrdering('type')" :class="orderByIcon('type')"></i>
                    <span v-html="i18n.fields['type']"></span>
                </button>
                <button @click="orderBy('is_private')" :class="orderByClass('is_private')" class="px-2 py-2 hover:bg-base-200 rounded-xl flex items-center gap-1.5 text-sm transition-all duration-150  text-base-content">
...
const initTippyDropdown = (btnRef, menuRef, placement = 'bottom') => {
