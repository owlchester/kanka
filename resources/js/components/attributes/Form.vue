<template>
    <div class="flex gap-5 lg:gap-10 justify-center justify-items-stretch items-center
     lg:border lg:rounded bg-base-200 lg:bg-inherit p-2 lg:p-5 text-xs md:text-sm m-0 lg:m-5 ">
        <div v-if="showAddForm" role="button" @click="addAttribute('')" class="flex flex-col gap-1 items-center">
            <i class="fa-solid fa-person text-xl md:text-2xl" aria-hidden="true" />
            <span v-html="trans('types.attribute')" class="text-xs"></span>
        </div>

        <div v-if="showAddForm" role="button" @click="addAttribute('multiline')" class="flex flex-col gap-1 items-center">
        ...
        <div v-if="showAddForm" role="button" @click="addAttribute('number')" class="flex flex-col gap-1 items-center">
        ...
        <div v-if="showAddForm" role="button" @click="addAttribute('section')" class="flex flex-col gap-1 items-center">
            <i class="fa-solid fa-layer-group text-xl md:text-2xl" aria-hidden="true" />
            <span v-html="trans('types.section')" class="text-xs"></span>
        </div>

        <div v-if="showAddForm" role="button" @click="addAttribute('checkbox')" class="flex flex-col gap-1 items-center">
        ...
        <div v-if="showAddForm" role="button" @click="addAttribute('random')" class="flex flex-col gap-1 items-center">
