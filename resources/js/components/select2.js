...
const resolveDropdownParent = (field) => {
    const attr = field.dataset.dropdownParent;
    if (attr) {
    ...
    return field.closest('dialog') ? 'body' : null;
};
...
window.initForeignSelect = function () {
    document.querySelectorAll("select.select2").forEach((field) => {
        ...
        const dropdownParent = resolveDropdownParent(field);
                ...
                option: function (data, escape) {
                    if (data.image) {
                        return (
                            ...
                            '<img src="' +
                            escape(data.image) +
                            '" class="rounded-full flex-none w-6 h-6"/>' +
                            '<span class="grow">' +
                            escape(data.text) +
                            "</span>" +
                            "</div>"
        ...
        new TomSelect(field, {
            ...
            render: {
                option: function (data, escape) {
                    ...
                    return (
                        '<div class="flex items-center gap-2"><span class="badge label bg-' +
                        escape(data.value) +
                        ' inline-block w-4 h-4 rounded-sm mr-1"> </span>' +
                        escape(data.text) +
                        "</div>"
                    );
                },
