<?php

return [
    'about'                 => [
        'title' => 'Acerca de',
    ],
    'actions'               => [
        'back-to-top'   => 'Volver al principio',
    ],
    'campaigns'             => [
        'description_full'  => 'Muchas de las campañas de Kanka son públicas para que todos las puedan ver. Esta página contiene una lista con todas las campañas públicas, así como algunas campañas destacadas de la comunidad.',
        'featured'          => [
            'description'   => 'Nuestra comunidad construye campañas increíbles que hemos incluido en las Campañas Destacadas. Iremos cambiando esta lista con frecuencia, así que asegúrate de volver de vez en cuando para ver qué novedades hay.',
            'title'         => 'Campañas destacadas',
        ],
        'open'              => [
            'filters'   => [
                'all'       => 'Estatus de apertura',
                'closed'    => 'Campañas cerradas',
                'featured'  => 'Ganadores del concurso',
                'open'      => 'Campañas abiertas',
            ],
        ],
        'public'            => [
            'description'   => 'Échale un vistazo a las campañas públicas para inspirarte.',
            'filters'       => [
                'all'           => 'Estatus de mejora',
                'entities'      => 'Ordenar por número de entidades',
                'followers'     => 'Ordenar por seguidores',
                'is-premium'    => '¡Esta es una campaña premium!',
                'premium'       => 'Campañas Premium',
                'standard'      => 'Campañas estándar',
            ],
            'no-results'    => 'No se encuentra ninguna campaña pública con estos filtros. Por favor, vuelve a intentarlo.',
            'title'         => 'Campañas públicas',
        ],
        'title'             => 'Campañas públicas',
    ],
    'community'             => [],
    'contact'               => [
        'description'   => '¿Necesitas ayuda, tienes una sugerencia o simplemente quieres compartir tu amor por Kanka? ¡Nuestro equipo te responderá en cualquier momento!',
        'email'         => 'Envía cualquier consulta a',
        'title'         => 'Estamos aquí para ayudarte',
    ],
    'cookie'                => [
        'dismiss'   => '¡Entendido!',
        'link'      => 'Saber más',
        'message'   => 'Esta página web usa cookies para asegurarse de que obtienes la mejor experiencia.',
    ],
    'faq'                   => [
        'helpers'   => [
            'more'  => '¿No encuentras lo que buscas?',
        ],
    ],
    'featured_campaigns'    => [
        'description'   => '¿Quieres ver a Kanka en acción y cómo lo utilizan otras personas? Aquí tienes algunas campañas de nuestra comunidad que nos parecen absolutamente increíbles.',
        'more'          => '¿Quieres ver más campañas? Echa un vistazo a nuestras :public-campaigns.',
        'title'         => '¿Buscas inspiración?',
    ],
    'features'              => [
        'abilities'         => [
            'description'   => 'Gestiona los inventarios de la entidad. Añade hechizos y poderes a los personajes mediante habilidades.',
            'title'         => 'Habilidades y objetos',
        ],
        'actions'           => [
            'rpg'           => 'Para campañas de rol',
            'worldbuilder'  => 'Para creadores',
        ],
        'api'               => [
            'description'   => '¿Quieres crear herramientas que se integren en Kanka? Lo tienes todo en :link',
            'link'          => 'Documentación API',
            'title'         => 'API',
        ],
        'audio'             => [
            'description'   => 'Sin necesidad de estar pendiente de 10 listas de reproducción; simplemente inserta tu audio y vídeo directamente en las entidades.',
            'title'         => 'Audio y vídeo insertado',
        ],
        'boosts'            => [
            'beta'                  => 'Acceso anticipado a algunas funciones nuevas 30 días antes del lanzamiento general.',
            'css'                   => 'CSS personalizado de la campaña',
            'dashboards'            => 'Configura tableros personalizados para los diferentes roles de la campaña',
            'description'           => 'Algunos niveles de mecenazgo otorgan mejoras a los usuarios, que pueden usar para mejorar campañas. Éstas obtienen los siguientes beneficios.',
            'entity-aliases'        => 'Establece alias en las entidades para flexibilizar la búsqueda y las menciones',
            'entity_files_v2'       => 'Máximo de archivos cargados por entidad',
            'entity_links'          => 'Enlaces a recursos y webs externas en las entidades',
            'header_image'          => 'Imagen de cabecera para las entidades',
            'images'                => 'Imágenes por defecto en las entidades',
            'map_markers'           => 'Acceso a miles de iconos para marcadores cartográficos y formas de marcador personalizadas',
            'marketplace'           => 'Acceso al :marketplace',
            'modules'               => 'Renombra módulos, por ejemplo cambia familias por clanes en una campaña.',
            'no_ads'                => 'Sin anuncios en la campaña',
            'premium'               => 'Campaña Premium',
            'recovery'              => 'Recuperación de entidades de hasta 30 días',
            'relation-visualiser'   => 'Explora visualmente las relaciones entre entidades',
            'sidebar'               => 'Reordena y personaliza la barra lateral de una campaña',
            'standard'              => 'Campaña estándar',
            'theme'                 => 'Tema de campaña fijo',
            'timeline_elements'     => 'Personaliza el icono y el color de los elementos de la línea de tiempo',
            'tooltip'               => 'Descripciones emergentes personalizadas',
            'upload'                => 'Subida de archivos ampliada para todos los miembros',
        ],
        'boosts_v2'         => [
            'description'       => 'Nuestros niveles de membresia te dan acceso a potenciadores, que puedes utilizar para potenciar o superpotenciar campañas. Al hacerlo, desbloqueas las siguientes ventajas para una campaña.',
            'description-count' => 'Para impulsar una campaña se utiliza :boost-count booster. Superimpulsar una campaña utiliza :superboost-count boosters.',
            'moving'            => 'Los potenciadores pueden moverse de una campaña a otra. Por ejemplo, si una campaña ha finalizado, puedes quitarle el potenciador y potenciar o superpotenciar otra campaña en su lugar. Al despotenciar una campaña no se eliminan los datos relacionados con las funciones potenciadas, sino que simplemente se ocultan hasta que se vuelve a potenciar la campaña.',
            'title'             => 'Campañas potenciadas y superpotenciadas',
        ],
        'calendars'         => [
            'description'   => 'Crea calendarios personalizados y organiza tu campaña con ellos.',
            'title'         => 'Calendarios',
        ],
        'collaborative'     => [
            'title' => 'Colaborativo',
        ],
        'dashboards'        => [
            'title' => 'Tableros',
        ],
        'description'       => 'Kanka tiene una gran cantidad de funciones. Puedes hacer un seguimiento de tus personajes y sus relaciones, los lugares y su historia, las misiones y sus objetivos, los calendarios y sus eventos y fenómenos meteorológicos, y mucho más. Lo mejor de todo es que Kanka recibe muchas actualizaciones cada mes basadas en los comentarios de los usuarios.',
        'description_full'  => 'Kanka se actualiza constantemente, pero aquí hay algunas de las más importantes que creemos que te van a encantar.',
        'flora'             => [
            'description'   => 'Lleva la cuenta de las criaturas vivientes de tu mundo con el módulo de Razas, y crea un complejo ecosistema usando las Notas anidadas.',
            'title'         => 'Flora y fauna',
        ],
        'free'              => [
            'bonuses'   => 'Bonus',
            'title'     => 'Uso Gratuito',
        ],
        'landscape'         => 'Por favor, pon tu dispositivo en modo apaisado para mostrar toda la lista de características.',
        'layers'            => [
            'description'   => 'Monitoriza tus personajes, sus familias su situación y mucho más en cualquier momento.',
            'title'         => 'Personajes, Familias, Localizaciones',
        ],
        'learn_more'        => 'Saber más sobre las funciones de Kanka',
        'learn_more_about'  => 'Saber más sobre esta funcionalidad',
        'maps'              => [
            'description'   => 'Sube mapas de tus mundos, ciudades y cuevas, y añádeles prácticos marcadores y enlaces.',
            'title'         => 'Mapas',
        ],
        'modular'           => [
            'description'   => 'Cada módulo (personajes, localizaciones) puede habilitarse o deshabilitarse en cualquier momento, facilitándote la personalización de tu campaña.',
            'title'         => 'Modular',
        ],
        'notebook'          => [
            'description'   => '¡Prepara tu siguiente sesión o registra lo ocurrido con nuestro editor de Diarios!',
            'title'         => 'Diarios',
        ],
        'patreon'           => [
            'api_calls'         => 'Llamadas a la API aumentadas (90)',
            'boosts'            => 'Mejoras de campaña',
            'bragi'             => 'Acceso a Bragi, la IA de construcción de mundos de Kanka',
            'click-me'          => 'Obtenga más información sobre esta función.',
            'curation'          => 'Ayuda a crear la lista mensual de votos',
            'default_image'     => 'Imágenes bonitas por defecto para las entidades',
            'description'       => 'Apoyar a Kanka con una pequeña tarifa mensual nos permite seguir trabajando en ella y desbloquea los siguientes beneficios.',
            'discord'           => 'Canal privado de Discord',
            'entities'          => 'Cantidad de entidades que se pueden crear en cada campaña',
            'free'              => 'Gratis',
            'hall_of_fame'      => 'Nombre en el :link',
            'impact'            => 'Influencia en futuras características',
            'monthly_vote'      => 'Participación en la votación mensual de características',
            'no_ads'            => 'No se mostrarán anuncios',
            'pagination'        => 'Aumento en los resultados por página (100)',
            'pagination_help'   => 'Este es el número máximo de resultados mostrados por página, NO un límite en el número de entidades por campaña.',
            'title'             => 'Funciones de pago',
            'upload_limit'      => 'Aumento del tamaño máximo de subida de archivos (8mb)',
            'upload_limit_map'  => 'Aumento del tamaño máximo de subida de mapas (10mb)',
            'users_roles'       => 'Cantidad de usuarios, roles y permisos',
        ],
        'public'            => [
            'description'   => 'Se pueden establecer permisos en los módulos, o en cada entidad por separado. Las campañas también pueden hacerse públicas para que <a href=":url">todo el mundo las vea</a>.',
            'title'         => 'Permisos',
        ],
        'quests'            => [
            'description'   => '¡Prepara tu siguiente sesión o registra los eventos con nuestra función avanzada de diarios!',
            'title'         => 'Misiones y diarios',
        ],
        'relations'         => [
            'description'   => 'Cada entidad puede tener una complicada red de relaciones con otras entidades, incluyendo relaciones secretas, invisibles para tus jugadores.',
            'title'         => 'Relaciones',
        ],
        'rpg'               => [
            'description'   => 'Desde un sistema de permisos muy detallado, hasta Misiones, Diarios, Personajes, Pabilidades e inserciones de audio y vídeo, te ayudamos a gestionar tu campaña.',
            'title'         => 'Gestión de campañas de rol',
        ],
        'secrets'           => [
            'description'   => 'Cada entidad puede tener asociados muchos secretos en forma de notas de entidad. Puedes ajustar los permisos de cada una para controlar quién la ve, y se pueden fijar a la vista principal de la entidad para acceder a ellas de un vistazo.',
            'title'         => 'Gestionar secretos',
        ],
        'superboosts'       => [
            'family-tree'   => 'Crea árboles genealógicos para seguir la historia de una familia',
            'gallery'       => 'Galería de campaña e integración con el editor de texto Summernote',
            'logs'          => 'Registros completos de los cambios de las entidades (de hasta 30 días)',
            'stats'         => 'Página de logros de campaña',
        ],
        'timelines'         => [
            'description'   => 'Representa visualmente las líneas de tiempo de tus mundos, imperios, familias nobles o hasta de un solo personaje.',
            'title'         => 'Líneas de tiempo',
        ],
        'title'             => 'Poderosas funciones con increíble flexibilidad',
        'unlimited'         => 'Ilimitado',
        'updates'           => [
            'title' => 'Actualizaciones frecuentes',
        ],
        'worldbuilding'     => [
            'title' => 'Creación de mundos',
        ],
    ],
    'first_block'           => [
        'description'   => 'Kanka está disponible en cualquier dispositivo móvil y navegador. No se necesita instalar nada, y siempre tendrás la última versión con las nuevas funciones. Olvídate de perderte entre notas desperdigadas: en Kanka, todo está en un solo lugar fácil de encontrar.',
        'title'         => 'Una nueva manera de monitorizar tus campañas.',
    ],
    'footer'                => [
        'copyright' => 'Copyright :copy :year :company',
        'headings'  => [
            'app'       => 'App',
            'community' => 'Comunidad',
        ],
    ],
    'goodbye'               => [
        'description_1' => 'Lamentamos tu marcha y entendemos que eliminar tu cuenta es una gran decisión. Te agradecemos el tiempo que pasaste con Kanka y esperamos que cumpliera tus expectativas.',
        'description_2' => 'Por favor, ten en cuenta que nuestro equipo se compromete a mejorar y actualizar constantemente Kanka para ofrecerte la mejor experiencia posible. Valoramos tus comentarios y sugerencias, y te animamos a ponerte en contacto con nosotros en :email en cualquier momento con cualquier comentario que puedas tener.',
        'description_3' => 'También queremos que sepas que si alguna vez decides volver, estaremos encantados de recibirte de nuevo. Estamos seguros de que, con nuestras frecuentes actualizaciones, Kanka te parecerá aún mejor y más fácil de usar que nunca.',
        'description_4' => 'Gracias de nuevo por formar parte de nuestra comunidad. Te deseamos lo mejor en tus futuros proyectos de construcción de mundos.',
        'title'         => 'Adiós',
    ],
    'help'                  => [
        'description'   => 'Estamos aquí para echarte una mano. Contáctanos usando por una de las siguientes vías, y te responderemos lo más rápido posible.',
        'discord'       => 'Únete a la comunidad en Discord',
        'documentation' => 'Consulta la documentación',
        'email'         => 'Envíanos un mail',
        'facebook'      => 'Contáctanos en Facebook',
        'title'         => 'Ayuda',
        'twitter'       => 'Tuitéanos en Twitter',
    ],
    'home'                  => [
        'seo'   => [
            'meta-description'  => 'Kanka es una herramienta de gestión de campañas de rol y creación de mundos, que facilita la organización y planificación de tus campañas RPG.',
        ],
    ],
    'master'                => [
        'call_to_action'        => 'Regístrate ahora, ¡es gratis!',
        'description'           => 'Kanka es una herramienta fácil y flexible para crear mundos y monitorizar tu campaña RPG en nuestra web. Crea y organiza a tus personajes, lugares, organizaciones, eventos y mucho más.',
        'description_q1_2023'   => ':kanka es una herramienta colaborativa de construcción de mundos perfecta para dar rienda suelta a tu creatividad. Construye tu mundo con entidades tipo wiki, calendarios complejos y hermosos mapas. Necesites lo que necesites, :kanka está aquí para ayudarte a crear y dirigir tu campaña de RPG casera.',
        'heading'               => 'Gestión de campañas de rol y creación de mundos a tu alcance',
        'title'                 => 'Kanka',
    ],
    'media'                 => [],
    'menu'                  => [
        'about'         => 'Acerca de',
        'campaigns'     => 'Campañas Públicas',
        'community'     => 'Comunidad',
        'contact'       => 'Contacto',
        'dashboard'     => 'Tablero',
        'documentation' => 'Documentación',
        'features'      => 'Características',
        'kb'            => 'Base de conocimientos',
        'login'         => 'Iniciar sesión',
        'marketplace'   => 'Marketplace',
        'merch'         => 'Merchandising',
        'news'          => 'Novedades',
        'newsletter'    => 'Newsletter',
        'overview'      => 'Funcionalidades principales',
        'pricing'       => 'Tarifas',
        'privacy'       => 'Política de Privacidad',
        'register'      => 'Registrarse',
        'register_free' => 'Regístrate gratis',
        'releases'      => 'Lanzamientos',
        'roadmap'       => 'Hoja de ruta',
    ],
    'meta'                  => [
        'description'   => 'Kanka es un administrador flexible de campañas de rol online.',
        'title'         => 'Kanka - Gestiona partidas de rol y crea mundos online',
    ],
    'partners'              => [
        'description'   => 'Nos hemos asociado con algunas herramientas asombrosas y con comunidades que comparten nuestros objetivos y valores.',
        'title'         => 'Socios',
    ],
    'pricing'               => [
        'actions'       => [
            'subscribe' => 'Suscribirse',
        ],
        'benefits'      => [
            'big_maps'      => 'Mapas grandes (:size)',
            'bigger_maps'   => 'Mapas más grandes (:size)',
            'boosters'      => 'Mejoras de campaña',
            'core'          => 'Acceso a todas las funciones principales',
            'huge_maps'     => 'Mapas enormes (:size)',
            'huge_uploads'  => 'Subir archivos enormes',
            'unlimited'     => 'Campañas y archivos subidos ilimitados',
            'updates'       => 'Actualizaciones y mejoras frecuentes',
        ],
        'billed_yearly' => ':amount facturada anualmente',
        'lead'          => [
            'obvious'   => 'elección obvia',
            'text'      => 'La :obvious para la construcción colaborativa de mundos y la gestión de campañas ttrpg, a un precio asequible.',
        ],
        'refund'        => 'Con una política de reembolso de 14 días para las suscripciones anuales.',
        'tier'          => [
            'free'  => 'Gratis',
            'month' => 'Mes',
        ],
        'title'         => 'Resumen de tarifas',
    ],
    'privacy'               => [
        'description'   => 'Fecha efectiva: :date',
        'title'         => 'Política de privacidad',
    ],
    'release'               => [],
    'roadmap'               => [
        'backlog'           => [
            'description'   => 'Este tablón de Trello contiene todas las peticiones de características propuestas por la comunidad, así como las mejoras planeadas que algún día llegarán a la app.',
            'title'         => 'Trabajos pendientes',
        ],
        'description'       => 'Los votaciones de nuestra comunidad moldean nuestro trabajo, así que, aunque no tenemos una hoja de ruta a meses vista, es sencillo estar al tanto de lo que se ha pedido y en lo que se está trabajando en cada momento.',
        'featured_requests' => [
            'title' => 'Peticiones destacadas',
        ],
        'next'              => [
            'description'   => 'Este tablón de Trello contiene todas las mejoras y características nuevas en las que estamos trabajando actualmente.',
            'title'         => 'Siguiente',
        ],
        'ongoing_progress'  => [
            'title' => 'Progreso en curso',
        ],
        'title'             => 'Hoja de ruta',
    ],
    'second_block'          => [
        'call_to_action'    => '¡Registra tu cuenta gratuita!',
    ],
    'seo'                   => [
        'keywords'  => 'Worldbuilding, Creación de mundos, RPG, Rol, Juego de rol, Gestión de campaña de rol',
    ],
    'team'                  => [
        'coffee'    => '¡Invítame a un café!',
    ],
    'terms'                 => [
        'description'   => 'Fecha efectiva: :date',
        'title'         => 'Términos y condiciones',
    ],
];
