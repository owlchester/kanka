<?php

return [
    'actions'   => [
        'current'   => 'Estilo actual: :theme',
        'disable'   => 'Desactivar',
        'enable'    => 'Activar',
        'new'       => 'Novo estilo',
    ],
    'bulks'     => [
        'delete'    => '{1} :count estilo eliminado.|[2,*] :count estilos eliminados.',
        'disable'   => '{1} :count estilo desactivado.|[2,*] :count estilos desactivados.',
        'enable'    => '{1} :count estilo activado.|[2,*] :count estilos activados.',
    ],
    'create'    => [
        'success'   => 'Novo estilo creado.',
        'title'     => 'Novo estilo',
    ],
    'delete'    => [
        'success'   => 'Estilo ":name" eliminado',
    ],
    'errors'    => [
        'max_reached'   => 'Chegaches ao número máximo de estilos (:max).',
    ],
    'fields'    => [
        'content'       => 'Regla CSS',
        'is_enabled'    => 'Activado',
        'length'        => 'Lonxitude',
        'modified'      => 'Modificado',
        'name'          => 'Nome',
        'order'         => 'Orde',
    ],
    'helpers'   => [
        'css_moved' => 'O campo CSS moveuse a unha interface máis flexible que pode ser atopada no menú da campaña :link.',
        'here'      => 'no noso blogue',
        'main'      => 'Podes crear estilos CSS personalizados para as túas campañas potenciadas. Estes estilos son cargados despois de calquera estilo importado do mercado que estea habilitado na campaña. Podes aprender máis sobre como estilar a túa campaña :here.',
    ],
    'pitch'     => 'Crea os teus propios estilos CSS para personalizar completamente a estética da túa campaña.',
    'reorder'   => [
        'save'      => 'Gardar nova orde',
        'success'   => '{1} :count estilo reordenado.|[2,*] :count estilos reordenados.',
        'title'     => 'Reordenar estilos',
    ],
    'theme'     => [
        'success'   => 'Estilo da campaña actualizado.',
        'title'     => 'Actualizar o estilo da campaña',
    ],
    'title'     => 'Estilos de campaña',
    'update'    => [
        'success'   => 'Estilo ":name" actualizado.',
        'title'     => 'Actualizar estilo',
    ],
];
