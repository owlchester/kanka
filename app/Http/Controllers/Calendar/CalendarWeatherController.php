...
class CalendarWeatherController extends Controller
{
    ...
    public function store(AddCalendarWeather $request, Campaign $campaign, Calendar $calendar)
    {
        ...
        $routeOptions = [$campaign->id, $calendar->id, 'year' => $weather->year, 'month' => $weather->month];
     ...
     * @throws \Illuminate\Auth\Access\AuthorizationException
     */
    public function destroy(Calendar $calendar, Campaign $campaign, CalendarWeather $calendarWeather)
    {
        $this->authorize('update', $calendar);
        $calendarWeather->delete();

        $routeOptions = [$campaign->id, $calendar->id];
        if (request()->has('layout')) {
            $routeOptions['layout'] = request()->get('layout');
        }
